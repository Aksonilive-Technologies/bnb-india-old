...
import { createBlogPost } from '@/actions/blog/blog.action';
...
const RichTextEditor = dynamic(() => import('@/components/shared/RichTextEditor'), {
    ssr: false,
    loading: () => <p>Loading...</p>, // Optional: Loading state while component is being loaded
});
...
export default function CreateBlogPage() {
    ...
    const handleUploadImage = async (src: any) => {
        try {

            // Create a storage reference for the image
            const imageRef = ref(storage, `images/${Date.now()}-${src}`);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (!blogData.title || !blogData.blogContent) {
                toast.error('Please fill in all required fields.');
                return;
            }

            // Handle image upload and get the URL
            const url = await handleUploadImage(blogData.image);
            ...
            if (url) {
                const data: any = {
                    ...
                    imageUrl: url,
                    tags: selectedTags, questions: blogData.questions
                };
                ...
                if (response.success) {
                    ...
                    console.error('Failed to create blog post', response.error);
                }
            }
            else {
                console.log('Failed to create blog url:');
            }
        } catch (error) {
            ...
            <SwitchTransition mode="out-in">
                ...
                >
                    <div>
                        {
                            !preview ?
                                <div className="w-full mx-auto min-h-[85vh] flex flex-col items-left md:max-w-[75vw] justify-center p-6 bg-white ">
                                    {/* Title Section */}
                                    <div className="mb-8">
                                    ...
                                    <div className="mb-4">
                                        <div className="flex flex-row items-center justify-between">
                                            <div className='w-[70%]'>
                                                <h1 className="mb-2 text-lg md:text-xl font-bold text-gray-800">Common Questions & Answers</h1>
                                            ...
                                            <div className="flex items-center justify-center">
                                                <button
                                                    ...
                                                    className="bg-gray-200 text-gray-800 font-semibold rounded-md p-2 hover:red-gradient hover:text-white"
                                                >
                                                    + Questions
                                                </button>
                                            </div>
                                ...
                                <main className="px-4 mx-auto md:w-[85vw] md:px-6 p-4 flex justify-center items-center">
                                    <article className="flex flex-col gap-6 max-w-screen-xl w-full sm:w-[95%] bg-white p-8 rounded-lg ">
                                        {/* Title Section */}
