...
import VillaCardSkeleton from "@/components/shared/villaCardSkeleton";
...
const ListingPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setNumberOfDays(
        (actualCheckout.getTime() - actualCheckin.getTime()) /
          (1000 * 60 * 60 * 24),
      );
  ...
  const filteredData = data
    ? data.filter((d) => {
        var { priceRange, bedrooms, beds, bathrooms } = filters;
...
export default function Listings() {
  return (
    <>
      <Suspense fallback={<VillaCardSkeleton />}>
        <ListingPage />
      </Suspense>
      <BottomNavBar />
    </>
